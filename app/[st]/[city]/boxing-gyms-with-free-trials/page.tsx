...
export async function generateMetadata({ params }: { params: Promise<{ st: string; city: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `Discover boxing gyms offering free trials in ${cityData.name}, ${stateData.state}. Try before you commit with these local boxing studios and martial arts centers.`,
  }
}

export default async function FreeTrialGymsPage({ params }: { params: Promise<{ st: string; city: string }> }) {
  const { st, city: citySlug } = await params
  const stateData = await getStateByAbbr(st)
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-12">
          ...
          <p className="text-xl text-gray-600 mb-6 max-w-3xl">
            Try before you commit! These {cityData.name} boxing gyms offer free trial classes so you can experience their training style, meet the coaches, and see if it's the right fit for your fitness journey.
          </p>
        ...
        <section className="mb-12">
          <h2 className="text-3xl font-bold text-black mb-8">
            {gyms.length > 0
              ? `${gyms.length} Boxing Gyms Offering Free Trials in ${cityData.name}`
