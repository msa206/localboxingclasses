...
DROP FUNCTION IF EXISTS public.lbc_gyms_near_zip CASCADE;

-- Recreate with correct types matching the lbc_boxing_gyms view
...
RETURNS TABLE (
  ...
  city text,
  state text,
  postal_code text,  -- Changed from bigint to text
  review_count bigint,
  review_stars double precision,
  working_hours text,  -- Changed from jsonb to text
...
BEGIN
  ...
  SELECT
    ...
    g.city::text,
    g.state::text,
    g.postal_code::text,  -- Now text
    g.review_count::bigint,
    g.review_stars::double precision,
    g.working_hours::text,  -- Now text
  ...
  FROM public.lbc_boxing_gyms g  -- Changed from lbc.boxing_gyms to public.lbc_boxing_gyms
  WHERE
